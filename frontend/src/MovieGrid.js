...
export default function MovieGrid() {
  ...
  useEffect(() => {
    const loadAllMovies = async () => {
      setLoading(true);
      try {
        const ids = Array.from({ length: 1963 }, (_, i) => i + 1);
        const query = ids.join(",");
        const response = await axios.get(`http://127.0.0.1:8000/movies?ids=${query}`);
  ...
  useEffect(() => {
    if (activeTab === 'recommended' && recommendedMovieIds.length > 0) {
      // Filter movies to show only recommended ones
      const recommendedMovies = allMovies.filter(movie =>
        recommendedMovieIds.includes(parseInt(movie.item_id || movie.movie_id))
      );
      ...
      <div className="grid grid-cols-5 gap-6">
        {filteredMovies.map((m) => (
          ...
          >
            ...
            />
            <div className="p-3">
              <h3 className="text-sm font-semibold truncate mb-1">{m.title}</h3>
