import pandas as pd
import ast
from utils import convert_age_to_string
...
df = pd.read_csv('data/movielens100k_details.csv')
...
def get_poster_url(posters):
    posters = ast.literal_eval(posters)
    if posters:
        return posters[-1]
    else:
        return "unknown"
...
n_ratings = compute_n_ratings("data/ml-100k.inter")
df['n_ratings'] = df['item_id'].map(n_ratings)
n_ratings_kid = compute_n_ratings("data/ml-100k.inter", user_meta_path="./data/ml-100k.user", category="kid")
df['n_ratings_kid'] = df['item_id'].map(n_ratings_kid).fillna(0).astype(int)
n_ratings_teenager = compute_n_ratings("data/ml-100k.inter", user_meta_path="./data/ml-100k.user", category="teenager")
df['n_ratings_teenager'] = df['item_id'].map(n_ratings_teenager).fillna(0).astype(int)
n_ratings_young_adult = compute_n_ratings("data/ml-100k.inter", user_meta_path="./data/ml-100k.user", category="young adult")
df['n_ratings_young_adult'] = df['item_id'].map(n_ratings_young_adult).fillna(0).astype(int)
n_ratings_adult = compute_n_ratings("data/ml-100k.inter", user_meta_path="./data/ml-100k.user", category="adult")
df['n_ratings_adult'] = df['item_id'].map(n_ratings_adult).fillna(0).astype(int)
n_ratings_senior = compute_n_ratings("data/ml-100k.inter", user_meta_path="./data/ml-100k.user", category="senior")
df['n_ratings_senior'] = df['item_id'].map(n_ratings_senior).fillna(0).astype(int)
n_ratings_male = compute_n_ratings("data/ml-100k.inter", user_meta_path="./data/ml-100k.user", category="M")
df['n_ratings_male'] = df['item_id'].map(n_ratings_male).fillna(0).astype(int)
n_ratings_female = compute_n_ratings("data/ml-100k.inter", user_meta_path="./data/ml-100k.user", category="F")
df['n_ratings_female'] = df['item_id'].map(n_ratings_female).fillna(0).astype(int)
df["poster_url"] = df["poster_url"].apply(get_poster_url)
ordered_columns = ['item_id', 'title', 'genres', 'director', 'producer', 'actors', 'release_date', 'release_month', 'country', 'duration', 'age_rating', 'imdb_rating', 'imdb_num_reviews', 'n_ratings', 'description', 'n_ratings_kid', 'n_ratings_teenager', 'n_ratings_young_adult', 'n_ratings_adult', 'n_ratings_senior', 'n_ratings_male', 'n_ratings_female', 'storyline', 'poster_url', 'movie_url', 'genres_list', 'directors_list', 'producers_list', 'actors_list']

# fill remaining NaN values with unknown
df.fillna("unknown", inplace=True)

# Save the final CSV
df.to_csv('data/final_ml-100k.csv', index=False, sep='\t', columns=ordered_columns)
